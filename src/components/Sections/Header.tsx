...
const Header: FC = memo(() => {
  ...
  const navSections = useMemo(
    () => [SectionId.About, SectionId.Portfolio, SectionId.Testimonials],
    [],
  );
...
const DesktopNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    const baseClass =
      '-m-1.5 p-1.5 rounded-md font-bold first-letter:uppercase hover:transition-colors hover:duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-neutral-100 sm:hover:text-red-500 text-neutral-100';
    const activeClass = classNames(baseClass, 'text-neutral-100');
    const inactiveClass = classNames(baseClass, 'text-neutral-100');
    return (
      <header className="fixed top-0 z-50 hidden w-full bg-neutral-1000/50 p-6 backdrop-blur sm:block" id={headerID}>
